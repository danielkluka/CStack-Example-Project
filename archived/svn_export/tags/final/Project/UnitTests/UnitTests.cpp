	...
	// c'tors
		...
		/**
		 \test **Test of CBlock conversion c'tor**. **Author:** `Kluka`.
		  - Instance can be correctly inicialized with `CBlock::TestValue0` and `CBlock::TestValue1`.
		...
		/**
		 \test **Test of String conversion c'tor**. **Author:** `Kluka`.
		  - Instance can be correctly inicialized with `CBlock::TestValue0` and `CBlock::TestValue1`.
		  - New Stacks are created using streams `"("+CBlock::TestStringValue0()+")"` and `"("+CBlock::TestStringValue1()+")"`.
		  - `CBlock::TestStringValue0` at the top of `stack0` is then compared with `CBlock::TestValue0` in instance `val0`, values must be same.
		  - `CBlock::TestStringValue1` at the top of `stack1` is then compared with `CBlock::TestValue1` in instance `val1`, values must be same.
		  - This is also test, if streams were given at the right format.
		  - Counter `ClassInfo::Living` was incremented.
		  - Each new CBlock instance have unique `ClassInfo::ID`.
		  - C'tor allocate, reallocate od deallocate dynamic memory if it was defined by `KCBlock_allocate` predicate.
		  - Destruction of constructed instance deallocate memory if it was defined by `KCBlock_allocate` predicate.
	...
	// unary operators
		...
		/**
		 \test **Test of Unary Operator -**. **Author:** `Kluka`.
		  ...
		  - If new Stacks have the same `Top` value after Operator - was used, Stacks weren't reversed.
		...
		/**
		 \test **Test of Unary Operator --**. **Author:** `Kluka`.
		  ...
		  - Methods `Push` and `PocetPrvku` are also used in this test.
		  - Counter `ClassInfo::Living` was incremented.
		  - Each new CBlock instance have unique `ClassInfo::ID`.
		  - C'tor allocate, reallocate od deallocate dynamic memory if it was defined by `KCBlock_allocate` predicate.
		  - Destruction of constructed instance deallocate memory if it was defined by `KCBlock_allocate` predicate.
	...
	// binary operators
		...
		/**
		 \test **Test of Binary Operator -=**. **Author:** `Kluka`.
		  ...
		  - Methods `Push`, `Search` and 'PocetPrvku' are also used in this test.
		  - Counter `ClassInfo::Living` was incremented.
		  - Each new CBlock instance have unique `ClassInfo::ID`.
		  - C'tor allocate, reallocate od deallocate dynamic memory if it was defined by `KCBlock_allocate` predicate.
		  - Destruction of constructed instance deallocate memory if it was defined by `KCBlock_allocate` predicate.
